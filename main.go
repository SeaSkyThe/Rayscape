...
func main() {
    ...
    var viewport_aux = vector.Vec3{
        X: camera_center.X - (viewport_u.X/2 + viewport_v.X/2),
        Y: camera_center.Y - (viewport_u.Y/2 + viewport_v.Y/2),
        Z: camera_center.Z - focal_length - (viewport_u.Z/2 + viewport_v.Z/2),
    }
	var viewport_upper_left = vector.Subtract(camera_center, viewport_aux)

	var pixel_delta_u_plus_v = vector.Add(pixel_delta_u, pixel_delta_v)
	var pixel00_loc = vector.Multiply(vector.Add(viewport_upper_left, vector.Vec3{X: 0.5, Y: 0.5, Z: 0.5}), pixel_delta_u_plus_v)
	...
	for j := 0; j < image_height; j++ {
		...
		for i := 0; i < image_width; i++ {
			...
			// pixel_color := color.Color3{X: float64(i) / float64(image_width-1), Y: float64(j) / float64(image_height-1), Z: 0.0}
